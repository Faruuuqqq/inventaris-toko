...
<div x-data="editManager()" class="max-w-2xl">
    <!-- Page Header -->
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-foreground">Edit Supplier</h2>
        <p class="mt-1 text-muted-foreground">Perbarui data supplier</p>
    </div>

    <!-- Form Card -->
    <div class="rounded-xl border border-border/50 bg-surface p-6">
        <form @submit.prevent="submitForm" action="<?= base_url('master/suppliers/' . $supplier->id) ?>" method="POST" class="space-y-5">
            <?= csrf_field() ?>
        ...
        async submitForm(event) {
            ...
            try {
                ...
                if (response.ok || response.status === 201) {
                    // Success
                    ModalManager.success('Data supplier berhasil diperbarui', () => {
                        // Redirect back to list
                        window.location.href = `<?= base_url('master/suppliers') ?>`;
                    });
